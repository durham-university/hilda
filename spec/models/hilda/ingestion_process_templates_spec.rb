...
RSpec.describe Hilda::IngestionProcessTemplate do
  ...
  describe "#build_process" do
    ...
    it "copies graph" do
      ...
      expect(mod_a.param_defs[:moo]).to eql({ label: 'moo', type: :string, default: nil, group: nil })
      expect(mod_a.param_defs[:baa]).to eql({ label: 'baa', type: :string, default: 'baa', group: nil })
